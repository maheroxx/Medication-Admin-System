...
if (!$conn) {
   ...
   $select = mysqli_query($conn, "SELECT * FROM `nurse` WHERE username = '$username' AND password = '$password'") or die('query failed');
   ...
   if(mysqli_num_rows($select) > 0){
      ...
      if($password != $cpassword){
		...
		window.location.href='addNurse.php';
         ...
         $insert = mysqli_query($conn, "INSERT INTO `nurse`(name, surname, nric, dob, age, email, phone, address, bloodgroup, gender, username, password, image) VALUES('$name', '$surname', '$nric', '$dob', '$age', '$email', '$phone', '$address', '$bloodgroup', '$gender', '$username', '$password', '$image')") or die('query failed');
	...
	<div class="wrapper fadeInDown">
		<div id="formContent">
			...
			<form method="POST" enctype="multipart/form-data">
                ...
                <input type="password" name="password" placeholder="Password" class="fadeIn one" required>
                <input type="password" name="cpassword" placeholder="Confirm Password" class="fadeIn one" required>
