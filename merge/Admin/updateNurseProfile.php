...
if(isset($_POST['update_nurse'])){
   ...
   mysqli_query($conn, "UPDATE `nurse` SET name = '$update_name', surname = '$update_surname', nric = '$update_nric', dob = '$update_dob', email = '$update_email', phone = '$update_phone', address = '$update_address' WHERE id = '$NurseID'") or die('query failed');
   ...
   if(!empty($update_password) || !empty($new_password) || !empty($confirm_password)){
      if($update_password != $old_password){
         ...
         mysqli_query($conn, "UPDATE `nurse` SET password = '$confirm_password' WHERE id = '$NurseID'") or die('query failed');
   ...
   if(!empty($update_image)){
      if($update_image_size > 30000){
         ...
         $image_update_query = mysqli_query($conn, "UPDATE `nurse` SET image = '$update_image' WHERE id = '$NurseID'") or die('query failed');
...
<head>
	...
	<title>Update Nurse Profile</title>
